...
public class ThongBaoDAO extends GenreicDAO<ThongBaoModel> implements IThongBaoDAO {
	...
	public Integer save(ThongBaoModel thongBaoModel) {
		...
		String sql = "INSERT INTO `traineemanager`.`notifications` (`title`, `content`, `to_classes`,"
	...
	public Integer update(ThongBaoModel thongBaoModel) {
		String sql="UPDATE `traineemanager`.`notifications` SET "
				+ "`title` = ?, "
				+ "`content` = ?,"
				+ " `to_classes` = ?, "
				+ "`modified_date` = ?, "
	...
	public Boolean deleteThongBao(Integer id) {
		try {
			String sql = "UPDATE `traineemanager`.`notifications` SET `status` = -1 where (`id` = ?) ";
			update(sql,id);
		return true;
		} catch (Exception e) {
			e.printStackTrace();
